...
function apply(ctx, config) {
  ...
  // Live rotation probe: pickKey → upstream call → markFailed/retry (same pool machinery as agent/request)
  ctx.effect(() => ctx.webServer.register({
    kind: 'exact',
    ...
    handler: async (req, res) => {
      ...
      if (!provider || !pools.has(provider)) { sendJson(res, 404, { error: `no pool for '${provider}'` }); return }
      if (!baseURL) { sendJson(res, 400, { error: 'baseURL required' }); return }
      const pool = pools.get(provider)
      // reset cooldown so the probe starts from a clean rotation window
      pool.states = new Map(pool.keys.map(k => [k, { failCount: 0, cooldownUntil: 0 }]))
      pool.idx = 0
      const attempts = []
      let ok = false
      for (let i = 0; i < maxAttempts; i++) {
        ...
        try {
          ...
          if (!r.ok) {
            ...
            errMsg = t.slice(0, 180)
          }
        } catch (e) {
          status = 0
          errMsg = String(e && e.message ? e.message : e).slice(0, 180)
        }
        attempts.push({ attempt: i + 1, key: mask(key), status, error: errMsg || undefined })
        if (status >= 200 && status < 300) {
          markSuccess(provider, key)
          ok = true
          break
        }
        // treat auth/rate/server errors as rotation triggers (same spirit as agent/request-error)
        if (status === 401 || status === 403 || status === 429 || status >= 500 || status === 0) {
          markFailed(provider, key, String(status || 'TRANSPORT'))
          continue
        }
        // other 4xx: still rotate once to keep probe useful
        markFailed(provider, key, String(status))
      }
      sendJson(res, 200, {
        ok,
        provider,
        baseURL,
        attempts,
        states: Object.fromEntries([...pool.states].map(([k, s]) => [mask(k), { failCount: s.failCount, cooldownUntil: s.cooldownUntil }])),
      })
    },
  }), 'api-key-pool: verify route')
  ctx.effect(() => () => {
    lifetime.abort()
