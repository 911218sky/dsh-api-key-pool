...
function ensureStorageDir() {
	mkdirSync(join(dshHome(), "storages", "dsh-api-key-pool"), {
		recursive: true,
		mode: 448
	});
}
...
function isSameOriginRequest(req) {
	...
	if (typeof origin === "string" && origin) try {
		return new URL(origin).host === host;
	} catch {
		return false;
	}
	const referer = req.headers.referer;
	if (typeof referer === "string" && referer) try {
...
const RESERVED_ENV_NAMES = /* @__PURE__ */ new Set([
	"PATH",
	"HOME",
	"USER",
	"SHELL",
...
function assertSafeApiKeyEnv(name) {
	if (!/^[A-Z][A-Z0-9_]{0,62}$/.test(name)) throw new Error("apiKeyEnv must be UPPER_SNAKE_CASE");
	if (RESERVED_ENV_NAMES.has(name) || name.startsWith("NODE_") || name.startsWith("npm_")) throw new Error("apiKeyEnv name is reserved");
	if (!/(?:_API_KEY|_TOKEN|_KEY)$/.test(name)) throw new Error("apiKeyEnv must end with _API_KEY, _TOKEN, or _KEY");
}
function normalizeIpLiteral(hostname) {
	return hostname.toLowerCase().replace(/^\[|\]$/g, "");
}
function ipv4MappedToV4(host) {
	return /^::ffff:(\d{1,3}(?:\.\d{1,3}){3})$/i.exec(host)?.[1];
}
function isPrivateOrLocalHostname(hostname) {
	...
	if (host.startsWith("fc") || host.startsWith("fd") || host.startsWith("fe80") || host === "::1" || host.startsWith("::ffff:")) return true;
	return false;
}
...
*/
async function assertSafePublicBaseURL(baseURL) {
	let url;
	try {
		url = new URL(baseURL);
	} catch {
	...
	if (isPrivateOrLocalHostname(host)) throw new Error("baseURL must not target private/loopback hosts");
	if (isIP(host)) {
		if (isBlockedResolvedAddress(host)) throw new Error("baseURL must not target private/loopback hosts");
		return;
	}
	let answers;
	try {
	...
	for (const ans of answers) if (isBlockedResolvedAddress(ans.address)) throw new Error("baseURL resolves to a private/loopback address");
}
/** Map probe HTTP status to a coarse class — never return upstream bodies. */
function classifyVerifyStatus(status) {
...
function providersFromSettingsDescribe(ctx) {
	try {
		const settingsSvc = ctx && "get" in ctx && typeof ctx.get === "function" ? ctx.get("settings") : void 0;
		const describe = settingsSvc?.describe;
		...
		for (const row of descs) {
			...
			if (providers && typeof providers === "object" && !Array.isArray(providers)) return Object.keys(providers);
		}
	} catch (err) {
...
function sanitizeApiKeyEnv(name, provider) {
	if (name) try {
...
function readPersisted() {
	...
	try {
		if (!existsSync(primary) && existsSync(legacy)) {
			mkdirSync(dirname(primary), {
				recursive: true,
				mode: 448
			});
			renameSync(legacy, primary);
			try {
				chmodSync(primary, 384);
			} catch {}
		}
	} catch {
		try {
			if (existsSync(legacy) && !existsSync(primary)) writeSecretFile(primary, readFileSync(legacy, "utf8"));
		} catch {}
	}
	try {
...
var KeyPoolManager = class {
	...
	async applyKeyToEnv(provider, key) {
		const pool = this.pools.get(provider);
		if (!pool || !key) return;
		...
		try {
			...
			if (creds && typeof creds.set === "function") await Promise.resolve(creds.set(envName, key));
		} catch (err) {
	...
	*/
	...
	ensurePool(provider, apiKeyEnv, firstKey) {
		...
		} else if (apiKeyEnv) pool.env = sanitizeApiKeyEnv(apiKeyEnv, provider);
		return pool;
	}
	addKey(provider, key) {
...
function registerRoutes(ctx, manager, config = {}) {
	...
	if (!requireAuth) try {
		ctx.logger?.warn?.("[api-key-pool] requireAuthForMutations=false — pool admin routes are open to the network");
	} catch {}
...
async function handleVerify(req, res, manager, requireAuth) {
	...
	for (let i = 0; i < maxAttempts; i++) {
		...
		attempts.push({
			...
			key: maskKey(key),
			status,
			error: status >= 200 && status < 300 ? void 0 : errClass
		});
...
function apply(ctx, config = {}) {
	ctx.inject(["settings"], async (sctx) => {
		try {
			...
			log(ctx, "error", `settings.register(api-key-pool) failed — Settings section may stay hidden: ${String(err instanceof Error ? err.message : err)}`);
		}
	});
...
//#endregion
export { apply, assertSafeApiKeyEnv, assertSafePublicBaseURL, inject, isMutationAllowed, isRequestAuthorized, maskKey, name };
